...
### Summary
When mobile email formatting is still broken after paragraph cleanup, reduce signature formatting further and avoid multi-line signoff blocks.

### Details
The remaining formatting problem appears to be concentrated in the signature or quoted region. Even if the root issue is client-side quoting, the safest generator-side change is to avoid adding a multi-line signoff block and keep the outbound close as a single compact line.

### Suggested Action
Use a one-line signoff for autogenerated plain-text replies when client rendering remains fragile.

### Metadata
- Source: conversation
- Related Files: mail/autopilot-run.sh
- Tags: email, formatting, mobile, signoff, correction

---

## [LRN-20260320-023] best_practice

**Logged**: 2026-03-20T08:05:00-07:00
**Priority**: medium
**Status**: pending
**Area**: docs

### Summary
When a state-changing helper updates source data, it should trigger refresh steps for any derived dashboards or summaries.

### Details
The new opportunity import helper successfully updated `business/opportunities.csv`, but the derived business summaries stayed stale until refresh scripts were run manually. The fix was to trigger the business refresh wrapper automatically from the import helper.

### Suggested Action
For workflows with derived views, wire state-changing helpers to refresh downstream summaries automatically when practical.

### Metadata
- Source: conversation
- Related Files: business/import_opportunity.py, business/refresh_all.sh, business/dashboard.md, business/next_actions.md, business/state_summary.md
- Tags: refresh, derived-state, business-ops, automation
